...
static async #onRoll(event, target) {
    ...
    const abilName = target.getAttribute('data-ability')
    const compName = game.i18n.localize(`Rupture.Character.Competences.${abilName}`);
    const formula = "3d6*5"+"+"+actor.system.competences[abilName]; console.log(formula);
    const cible = 10;
    try {
        // 1. Lancer les dés
        ...
        const success = total >= cible;
        // 3. Préparer les données pour la template
        const data = {
            actingCharImg: actor.img ?? "icons/svg/mystery-man.svg",
            actingCharName: actor.name ?? "Inconnu",
            actingAbilName: abilName,
            introText: `${actor.name} tente un test de ${compName}`,
            text: `Jet : ${formula} → Résultat : <b>${total}</b>`
        };

        // 4. Créer et afficher le message via LiberChat
        await new LiberChat(actor)
            .withTemplate("systems/rupture/templates/roll-dommage.hbs")
            .withData(data)
            .withRolls([roll])
            .create()
            .then(c => c.display());
        ...
        ui.notifications.error(`Erreur lors du jet : ${err.message}`);
        console.error(err);
    }
}


  // Données pour ton template
 /* const chatData = {
  ...
  // Speaker valide (évite l'erreur "16-character ID")
  const speaker = ChatMessage.getSpeaker({ actor: this.actor });

  // Rendu du template + création du message de chat avec le roll
  const content = await renderTemplate("systems/rupture/templates/roll-dommage.hbs", chatData);
  ...
  await ChatMessage.create({
    ...
    rolls: [roll]   // Affiche le résultat de dés cliquable
  });*/
